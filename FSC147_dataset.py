...
def get_image_classes(class_file):
    class_dict = dict()#空字典 存储类别和标识符
    #以制表符分隔的文本文件，其中每一行都包含一个类别的标识符和名称
    with open(class_file, 'r') as f:#读文件 拿到的东西放到classes
        classes = [line.split('\t') for line in f.readlines()]
    
    for entry in classes:#循环 标识符entry[0]与类别名称entry[1]一起添加到class_dict字典中
        class_dict[entry[0]] = entry[1]
    
    return class_dict #返回字典包含图像类别的名称和标识符
#数据批处理
...
if __name__ == '__main__':
    ...
    dataset = FSC147Dataset(data_dir='/桌面/lll/AAcac3.0/dataset',
                            data_list='/桌面/lll/AAcac3.0/datalist/train.txt',
                            scaling=1.0,
                            main_transform=main_transform,
                            query_transform=query_transform)
