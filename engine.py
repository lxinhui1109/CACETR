...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for idx, sample in enumerate(data_loader):
        ...
        if epoch < 5:  # check if training process get stucked in local optimal.
            print(f"pre_sum:{dest.sum().item()}", f"gt_sum:{density_map.sum().item()}",
                  f"scale{dest.sum().item() * 10000 / (img.shape[-2] * img.shape[-1])}")  # 非零元素的像素数占整个图像像素数的比例
...
def visualization(cfg, model, dataset, data_loader, device, output_dir):
    ...
    for idx, sample in enumerate(data_loader):
        ...
        density_map = torch.nn.functional.interpolate(density_map, (h, w), mode='bilinear').squeeze(0).squeeze(
            0).cpu().numpy()
        gt_density = torch.nn.functional.interpolate(gt_density, (h, w), mode='bilinear').squeeze(0).squeeze(
            0).cpu().numpy()
