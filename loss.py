...
class L1Loss(nn.Module):
    def __init__(self, device=cfg.TRAIN.device, reduction='mean', downsampling_rate=32):
...
class InfoNCELoss(nn.Module):
    def __init__(self, device=cfg.TRAIN.device, downsampling_rate=32):
        super().__init__()
        self.downsampling_rate = downsampling_rate
        self.kernel = torch.ones(1, 1, downsampling_rate, downsampling_rate).to(device)
        ...
        # kernel_size = (32, 32)
        # stride = (32, 32)
        # self.kernel = torch.ones(1, 1, *kernel_size).to(device)
    ...
    def forward(self, corr_map, pt_map, density_map):
        ...
        # con = ConvDownsample().cuda()
        # pt_map = con(pt_map.float()).bool()
...
def get_loss(cfg):
    ...
    if cfg.TRAIN.counting_loss == 'l1loss':
        counting_loss = L1Loss(device=cfg.TRAIN.device, reduction='mean',
                               downsampling_rate=cfg.DATASET.downsampling_rate)
    elif cfg.TRAIN.counting_loss == 'l2loss':
        counting_loss = L2Loss(device=cfg.TRAIN.device, reduction='mean',
                               downsampling_rate=cfg.DATASET.downsampling_rate)
    else:
